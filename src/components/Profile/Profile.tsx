              ...
              <div className="flex flex-col gap-5 md:gap-0">
                {user.map((item, index) => (
                  <div key={index} className="flex gap-2">
                    <img className="w-[124px] h-[124px] md:w-[164px] md:h-[164px] rounded-lg shadow-3xl" src={item.avatar_url as string} alt={item.name} />

                    <div className="md:mt-2">
                      <h1 className="text-lg md:text-2xl font-bold">{item.name ? item.name : "Informação não encontrada."}</h1>
                      <p className="text-base font-medium text-[#787878]">
                        {item.login} &bull; <span>{dateFormat(item.created_at)}</span>
                      </p>
                      <p className="text-sm md:text-xl font-medium">{item.bio ? item.bio : "Informação não encontrada."}</p>
                      <div className="flex gap-2 mt-2 md:mt-3">
                        <a className="flex items-center gap-1 cursor-pointer font-medium transition-colors hover:text-gray-200" href={item.repos_url} target="_blank">
